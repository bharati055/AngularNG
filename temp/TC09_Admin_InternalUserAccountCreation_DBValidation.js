...
describe('Validate Internal User Account Creation-', function(){
	...
	it('It should fill User and Account Information Form.', function(){
		...
		browser.sleep(5000);
		data.accData.accountName = "Auto_User";
		logger.info("New Acc name is:" + data.accData.accountName);
		accCreationPage.addUserAccountInfo_internalUser();
	...
	it('Logging Out and logging in with the End user which we just created', function(){

		//browser.close();
		browser.sleep(5000);
		cf.navigateToURL();	
		browser.sleep(7000);
		loginPage.eu_usernameInput_fn().sendKeys(data.accData.accountName+"@pb.com");
		loginPage.eu_passwordInput_fn().sendKeys(env.spodApp.qa_eu.password);
        loginPage.eu_signinButton_fn().click();
		browser.sleep(15000);
		...
		logger.info("Successfully Logged in to End User portal with the new End user");
